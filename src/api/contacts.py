...
async def search_contacts(query: str, db: AsyncSession = Depends(get_db)) -> List[ContactResponse]:
    if not query:
...
async def create_contact(contact: ContactResponse, db: AsyncSession = Depends(get_db)):
    service = ContactService(db)
    return await service.create_contact(contact)


@router.put("/{contact_id}", response_model=ContactResponse)
async def update_contact(contact_id: int, contact: ContactResponse, db: AsyncSession = Depends(get_db)):
    service = ContactService(db)
